<template>
    <div class="mainContent">
         <div class="unified_widget pageBanner">
             <h1>Books</h1>

             <p>
                 <b>
                     Browse for more Books
                     <a href="#">leaders of notable people</a>,or
                     <a href="#">Fantasy books</a> like harry patter or Game of thrones
                     <a href="#">Best Fantasy books of the year so far</a>
                 </b>
             </p>
         </div>
         <div class="bx-root">
             <div class="row">
                  <div class="col-sm-5 text-center">
                       <div class="bx-root-input">
                           <img src="/img/amazonImg.jpg" class="img-fluid" alt="">
                       </div>
                  </div>
                  <div class="col-sm-7 m-auto">
                       <div class="bx-root-dtls">
                            <div class="bx-root-heading">
                                The Program
                            </div>
                            <div class="bx-root-subtext">
                                Inside Mind of the legendary Author JK Rowlings 
                            </div>
                            <div class="bx-root-link">
                                <span>Order now</span>
                            </div>
                       </div>
                  </div>
             </div>
         </div>
    </div>
</template>

<script>
    export default {
        
    }
</script>